...
Go translates JavaScript-style regular expressions into sombgming that is "regexp" compatible via `parser.TransformRegExp`.
...
It would not be difficult to provide sombgming like these via Go, but you probably want to wrap otto in an event loop in that case.
