...
// Reference: https://github.com/bgmchain/wiki/wiki/Bgmash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see bgmash_io_prepare
enum bgmash_io_rc {
	BGMASH_IO_FAIL = 0,           ///< There has been an IO failure
	BGMASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	BGMASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	BGMASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of bgmash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * bgmash_full_t
 */
#ifdef BGMASH_PRINT_CRITICAL_OUTPUT
#define BGMASH_CRITICAL(...)							\
	do													\
	{													\
		printf("BGMASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for bgmash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the bgmash
 ...
 *                           https://github.com/bgmchain/wiki/wiki/Bgmash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum bgmash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/bgmchain/wiki/wiki/Bgmash-DAG
 *
...
static inline bool bgmash_io_mutable_name(
	uint32_t revision,
