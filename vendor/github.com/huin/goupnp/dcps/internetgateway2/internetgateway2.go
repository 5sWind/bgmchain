...
const (
	...
	URN_WANBgmchainnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANBgmchainnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Bgmchainnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Bgmchainnet
//
...
// WANBgmchainnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANBgmchainnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANBgmchainnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANBgmchainnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANBgmchainnetLinkConfig1Clients() (clients []*WANBgmchainnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANBgmchainnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANBgmchainnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANBgmchainnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANBgmchainnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANBgmchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANBgmchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANBgmchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANBgmchainnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANBgmchainnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANBgmchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANBgmchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANBgmchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANBgmchainnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANBgmchainnetLinkConfig1 {
	clients := make([]*WANBgmchainnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANBgmchainnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewBgmchainnetLinkStatus: allowed values: Up, Down
func (client *WANBgmchainnetLinkConfig1) GetBgmchainnetLinkStatus() (NewBgmchainnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewBgmchainnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANBgmchainnetLinkConfig_1, "GetBgmchainnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewBgmchainnetLinkStatus, err = soap.UnmarshalString(response.NewBgmchainnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
