...
func (bgm *Bgmchain) startBloomHandlers() {
			...
			for {
				...
				case request := <-bgm.bloomRequests:
					...
					for i, section := range task.Sections {
						head := core.GetCanonicalHash(bgm.chainDb, (section+1)*params.BloomBitsBlocks-1)
						if compVector, err := core.GetBloomBits(bgm.chainDb, task.Bit, section, head); err == nil {
...
type BloomIndexer struct {
	...
	db  bgmdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db bgmdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := bgmdb.NewTable(db, string(core.BloomBitsIndexPrefix))
