...
import (
	...
	"github.com/5sWind/bgmchain/internal/jsre"
	"github.com/5sWind/bgmchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Bgmchain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jbgm", struct{}{})

	jbgmObj, _ := c.jsre.Get("jbgm")
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jbgm);"); err != nil {
	...
	//c.jsre.Run(`var GlobalRegistrar = bgm.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jbgm.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jbgm.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jbgm.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jbgm.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jbgm.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines bgm.getBalance(bgm.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	fmt.Fprintf(c.printer, "Welcome to the Gbgm JavaScript console!\n\n")
	c.jsre.Run(`
		console.log(" instance: " + web3.version.node);
		console.log("validator: " + bgm.validator);
		console.log(" coinbase: " + bgm.coinbase);
		console.log(" at block: " + bgm.blockNumber + " (" + new Date(1000 * bgm.getBlock(bgm.blockNumber).timestamp) + ")");
