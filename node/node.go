...
func (n *Node) OpenDatabase(name string, cache, handles int) (bgmdb.Database, error) {
	if n.config.DataDir == "" {
		return bgmdb.NewMemDatabase()
	}
	return bgmdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
