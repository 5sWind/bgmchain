...
// Package bgmash implements the bgmash proof-of-work consensus engine.
package bgmash
...
func (c *cache) generate(dir string, limit int, test bool) {
	c.once.Do(func() {
		...
		if err == nil {
			logger.Debug("Loaded old bgmash cache from disk")
			return
		}
		logger.Debug("Failed to load old bgmash cache", "err", err)

		// No previous cache available, create a new cache file to fill
		c.dump, c.mmap, c.cache, err = memoryMapAndGenerate(path, size, func(buffer []uint32) { generateCache(buffer, c.epoch, seed) })
		if err != nil {
			logger.Error("Failed to generate mapped bgmash cache", "err", err)
...
func (d *dataset) generate(dir string, limit int, test bool) {
	d.once.Do(func() {
		...
		if err == nil {
			logger.Debug("Loaded old bgmash dataset from disk")
			return
		}
		logger.Debug("Failed to load old bgmash dataset", "err", err)
		...
		if err != nil {
			logger.Error("Failed to generate mapped bgmash dataset", "err", err)
...
func New(cachedir string, cachesinmem, cachesondisk int, dagdir string, dagsinmem, dagsondisk int) *Bgmash {
	if cachesinmem <= 0 {
		log.Warn("One bgmash cache must always be in memory", "requested", cachesinmem)
		cachesinmem = 1
	}
	if cachedir != "" && cachesondisk > 0 {
		log.Info("Disk storage enabled for bgmash caches", "dir", cachedir, "count", cachesondisk)
	}
	if dagdir != "" && dagsondisk > 0 {
		log.Info("Disk storage enabled for bgmash DAGs", "dir", dagdir, "count", dagsondisk)
	}
	return &Bgmash{
		cachedir:     cachedir,
...
func (bgmash *Bgmash) cache(block uint64) []uint32 {
	epoch := block / epochLength

	// If we have a PoW for that epoch, use that
	bgmash.lock.Lock()

	current, future := bgmash.caches[epoch], (*cache)(nil)
	if current == nil {
		// No in-memory cache, evict the oldest if the cache limit was reached
		for len(bgmash.caches) > 0 && len(bgmash.caches) >= bgmash.cachesinmem {
			var evict *cache
			for _, cache := range bgmash.caches {
				if evict == nil || evict.used.After(cache.used) {
					evict = cache
				}
			}
			delete(bgmash.caches, evict.epoch)
			evict.release()

			log.Trace("Evicted bgmash cache", "epoch", evict.epoch, "used", evict.used)
		}
		// If we have the new cache pre-generated, use that, otherwise create a new one
		if bgmash.fcache != nil && bgmash.fcache.epoch == epoch {
			log.Trace("Using pre-generated cache", "epoch", epoch)
			current, bgmash.fcache = bgmash.fcache, nil
		} else {
			log.Trace("Requiring new bgmash cache", "epoch", epoch)
			current = &cache{epoch: epoch}
		}
		bgmash.caches[epoch] = current

		// If we just used up the future cache, or need a refresh, regenerate
		if bgmash.fcache == nil || bgmash.fcache.epoch <= epoch {
			if bgmash.fcache != nil {
				bgmash.fcache.release()
			}
			log.Trace("Requiring new future bgmash cache", "epoch", epoch+1)
			future = &cache{epoch: epoch + 1}
			bgmash.fcache = future
		}
	...
	current.generate(bgmash.cachedir, bgmash.cachesondisk, bgmash.tester)
	...
	if future != nil {
		go future.generate(bgmash.cachedir, bgmash.cachesondisk, bgmash.tester)
	}
...
// stored on disk, and finally generating one if none can be found.
func (bgmash *Bgmash) dataset(block uint64) []uint32 {
	epoch := block / epochLength

	// If we have a PoW for that epoch, use that
	bgmash.lock.Lock()

	current, future := bgmash.datasets[epoch], (*dataset)(nil)
	if current == nil {
		// No in-memory dataset, evict the oldest if the dataset limit was reached
		for len(bgmash.datasets) > 0 && len(bgmash.datasets) >= bgmash.dagsinmem {
			var evict *dataset
			for _, dataset := range bgmash.datasets {
				if evict == nil || evict.used.After(dataset.used) {
					evict = dataset
				}
			}
			delete(bgmash.datasets, evict.epoch)
			evict.release()

			log.Trace("Evicted bgmash dataset", "epoch", evict.epoch, "used", evict.used)
		}
		// If we have the new cache pre-generated, use that, otherwise create a new one
		if bgmash.fdataset != nil && bgmash.fdataset.epoch == epoch {
			log.Trace("Using pre-generated dataset", "epoch", epoch)
			current = &dataset{epoch: bgmash.fdataset.epoch} // Reload from disk
			bgmash.fdataset = nil
		} else {
			log.Trace("Requiring new bgmash dataset", "epoch", epoch)
			current = &dataset{epoch: epoch}
		}
		bgmash.datasets[epoch] = current

		// If we just used up the future dataset, or need a refresh, regenerate
		if bgmash.fdataset == nil || bgmash.fdataset.epoch <= epoch {
			if bgmash.fdataset != nil {
				bgmash.fdataset.release()
			}
			log.Trace("Requiring new future bgmash dataset", "epoch", epoch+1)
			future = &dataset{epoch: epoch + 1}
			bgmash.fdataset = future
		}
	...
	// Wait for generation finish, bump the timestamp and finalize the cache
	current.generate(bgmash.dagdir, bgmash.dagsondisk, bgmash.tester)
	...
	if future != nil {
		go future.generate(bgmash.dagdir, bgmash.dagsondisk, bgmash.tester)
	}
...
func (bgmash *Bgmash) Threads() int {
	bgmash.lock.Lock()
	defer bgmash.lock.Unlock()

	return bgmash.threads
}
...
// work being done.
func (bgmash *Bgmash) SetThreads(threads int) {
	bgmash.lock.Lock()
	defer bgmash.lock.Unlock()

	// If we're running a shared PoW, set the thread count on that instead
	if bgmash.shared != nil {
		bgmash.shared.SetThreads(threads)
		return
	}
	// Update the threads and ping any running seal to pull in any changes
	bgmash.threads = threads
	select {
	case bgmash.update <- struct{}{}:
	default:
	}
}
...
func (bgmash *Bgmash) Hashrate() float64 {
	return bgmash.hashrate.Rate1()
}

// APIs implements consensus.Engine, returning the user facing RPC APIs. Currently
// that is empty.
func (bgmash *Bgmash) APIs(chain consensus.ChainReader) []rpc.API {
	return nil
}
