...
type testOdr struct {
	OdrBackend
	sdb, ldb bgmdb.Database
	disable  bool
}

func (odr *testOdr) Database() bgmdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db bgmdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db bgmdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db bgmdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db bgmdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db bgmdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
//
	...
	lightchain, err := NewLightChain(odr, params.TestChainConfig, bgmash.NewFullFaker())
	if err != nil {
		t.Fatal(err)
	}
