...
//
	...
	configPrefix   = []byte("bgmchain-config-") //
...
//
	block.DposContext = getDposContextTrie(db.(bgmdb.Database), header)
	return block
}

func getDposContextTrie(db bgmdb.Database, header *types.Header) *types.DposContext {
...
//
func WriteCanonicalHash(db bgmdb.Putter, hash common.Hash, number uint64) error {
...
//
func WriteHeadHeaderHash(db bgmdb.Putter, hash common.Hash) error {
...
//
func WriteHeadBlockHash(db bgmdb.Putter, hash common.Hash) error {
...
//
func WriteHeadFastBlockHash(db bgmdb.Putter, hash common.Hash) error {
...
//
func WriteHeader(db bgmdb.Putter, header *types.Header) error {
...
//
func WriteBody(db bgmdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
//
func WriteBodyRLP(db bgmdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
//
func WriteTd(db bgmdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
//
func WriteBlock(db bgmdb.Putter, block *types.Block) error {
//
...
//
//
func WriteBlockReceipts(db bgmdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
//
...
//
//
func WriteTxLookupEntries(db bgmdb.Putter, block *types.Block) error {
//
...
//
//
func WriteBloomBits(db bgmdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
//
func PreimageTable(db bgmdb.Database) bgmdb.Database {
	return bgmdb.NewTable(db, preimagePrefix)
}

//
//
func WritePreimages(db bgmdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
//
func WriteBlockChainVersion(db bgmdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

//
func WriteChainConfig(db bgmdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
//
//
	if cfg == nil {
		return nil
