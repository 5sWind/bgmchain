...
import (
	...
	"github.com/5sWind/bgmchain/swarm"
	bzzapi "github.com/5sWind/bgmchain/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:  "swap-api",
		Usage: "URL of the Bgmchain API provider to use to settle SWAP payments",
	}
	...
	EnsAPIFlag = cli.StringFlag{
		Name:  "ens-api",
		Usage: "URL of the Bgmchain API provider to use for ENS record lookups",
		Value: node.DefaultIPCEndpoint("gbgm"),
	}
	...
	DeprecatedBgmAPIFlag = cli.StringFlag{
		Name:  "bgmapi",
...
// This init function sets defaults so cmd/swarm can run alongside gbgm.
func init() {
...
var app = utils.NewApp(gitCommit, "Bgmchain Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-bgmchain Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.bgmchain/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.bgmchain/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.bgmchain/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.bgmchain/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	// exit if the deprecated --bgmapi flag is set
	if ctx.GlobalString(DeprecatedBgmAPIFlag.Name) != "" {
		utils.Fatalf("--bgmapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := bgmclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if ensapi != "" {
			...
			ensClient = bgmclient.NewClient(client)
