...
// Package utils contains internal helper functions for bgmchain commands.
package utils
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: bgm.DefaultConfig.NetworkId,
	}
	...
	defaultSyncMode = bgm.DefaultConfig.SyncMode
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: bgm.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: bgm.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: bgm.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: bgm.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: bgm.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: bgm.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: bgm.DefaultConfig.TxPool.Lifetime,
	}
	...
	GasPriceFlag = BigFlag{
		...
		Value: bgm.DefaultConfig.GasPrice,
	}
	...
	BgmStatsURLFlag = cli.StringFlag{
		Name:  "bgmstats",
		Usage: "Reporting URL of a bgmstats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: bgm.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: bgm.DefaultConfig.GPO.Percentile,
	}
...
// for Gbgm and returns half of the allowance to assign to the database.
...
func setValidator(ctx *cli.Context, ks *keystore.KeyStore, cfg *bgm.Config) {
...
// command line flags or from the keystore if CLI indexed.
func setCoinbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *bgm.Config) {
...
// SetBgmConfig applies bgm-related command line flags to the config.
func SetBgmConfig(ctx *cli.Context, stack *node.Node, cfg *bgm.Config) {
...
// RegisterBgmService adds an Bgmchain client to the stack.
func RegisterBgmService(stack *node.Node, cfg *bgm.Config) {
	var err error
	...
	if err != nil {
		Fatalf("Failed to register the Bgmchain service: %v", err)
	}
}
...
// RegisterBgmStatsService configures the Bgmchain Stats daemon and adds it to
// th egiven node.
func RegisterBgmStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both bgm and les services
		var bgmServ *bgm.Bgmchain
		ctx.Service(&bgmServ)

		var lesServ *les.LightBgmchain
		ctx.Service(&lesServ)

		return bgmstats.New(url, bgmServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Bgmchain Stats service: %v", err)
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) bgmdb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb bgmdb.Database) {
	var err error
...
// new format.
//
// e.g. gbgm account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gbgm --keystore /tmp/mykeystore --lightkdf account new
//
