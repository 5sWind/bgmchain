...
# 1. install gbgm for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for gbgm
# 5. create gbgm, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable BGMCHAIN_SOCKET
...
!define GROUPNAME "Bgmchain"
!define APPNAME "Gbgm"
!define DESCRIPTION "Official Go implementation of the Bgmchain protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since gbgm is installed global
